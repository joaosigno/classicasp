...
Class Index
    ...
    Public Function listaPrincipalRssFlickr()
        ...
        Set r = new RssFlickr
        Dim tmp2
		If IsEmpty(Session("rssImagesA")) Then
			tmp2 = r.getPrincipalRssFeedElenco(recordsPerPage)
			Session("rssImagensA") = tmp2
		ElseIf DateDiff("n", Session("timeInit"), now) > 10 then
			tmp2 = r.getPrincipalRssFeedElenco(recordsPerPage)
			Session("rssImagensA") = tmp2
			Session("timeInit") = now
		End If
        listaPrincipalRssFlickr = Session("rssImagensA")
    End Function
	...
	Public Function listaRssFlickr(id)
        ...
        Set r = new RssFlickr
		If IsEmpty(Session("rssImagensA")) or Session("lastID") <> id Then
		...
		listaRssFlickr = Session("rssImagensA")
	End Function
