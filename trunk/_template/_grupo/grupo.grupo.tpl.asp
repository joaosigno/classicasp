...
<script type="text/javascript">
	...
	function listImagens(callBack) {
		...
		$_('imagemPrincipal').innerHTML = $_('listUltimasImagens3Raw').innerHTML;
...
<style type="text/css">
	...
	#titleImagens
	{
		width:150px;
		height:28px;
		background-image:url('./_img/bg_menu_top_grupo.jpg');
		background-repeat:no-repeat;
...
<span id="listUltimasImagens3Raw" style="display:none;visibility:hidden;">
	<span id="boxImage" mjt.for="a in images.rss" style="background-color:#FFF;">
		<span class="titleOfImage">${a.title}</span>
		<span class="descriptionOfImage">${mjt.bless(a.description)}</span>
		<span id="imagesFlickrPrincipal" mjt.onclick="window.open(a.image,'_blank');" style="cursor:pointer;">
			${mjt.bless('&lt;img src="' + a.imageSmall + '" /&gt;')}
		</span>
	</span>
</span>
